...
FUNCTION     : 20.15
FILE         : html_report.py
ROLE         : Génération du rapport HTML de sécurité
...
DESCRIPTION :
Génère le rapport HTML du Bloc 20 pour la démo et le tableau de bord de sécurité.
════════════════════════════════════════════════════════════
"""
...
def generate_html_report(
    ...
    """
    Génère le rapport HTML complet et retourne son chemin principal.

    Args:
        output_path  : destination principale (défaut : demo/alfred_security_report.html)
        extra_paths  : destinations supplémentaires où copier le rapport généré
                       (ex. ALFRED_WEB/templates/security_report.html)
    """
...
<footer>
  ...
  Bloc 20 Sécurité · {now.strftime('%Y')}
    ...
    # Copie vers les destinations supplémentaires
