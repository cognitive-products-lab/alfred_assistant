# ALFRED Roadmap

This roadmap is intentionally high-level and will evolve with the prototype.

## V1 — Foundation

- [x] Define ALFRED product vision
- [x] Define local-first principles
- [x] Create initial memory system
- [x] Create knowledge organization structure
- [x] Build first dashboard concept
- [x] Explore avatar visual identity
- [x] Explore emotional response logic
- [ ] Clean public repository structure
- [ ] Improve documentation
- [ ] Add public screenshots and diagrams
- [ ] Create first contributor-friendly issues
- [ ] Improve UX/UI prototype
- [ ] Clarify accessibility requirements

## V2 — Interaction & Experience

- [ ] Redesign main interface
- [ ] Create Figma UI concepts
- [ ] Build design system foundations
- [x] Improve avatar expression system (B15 — renderer 6 calques, halo, sprites par état)
- [x] Add simple animation states (idle/listening/thinking/speaking/support/focus + blink)
- [x] Improve dashboard readability (dashboards dynamiques B01-B22, avancement 63.6%)
- [ ] Explore Android interface
- [ ] Improve voice interaction flow (TTS streaming OK, grésillement à corriger)
- [x] Add accessibility modes (B22 — voice_output_manager, text_reader, WCAG checker)
- [ ] Document user journeys

## V3 — Multi-Device & Advanced Assistant

- [ ] Multi-device architecture
- [ ] Local synchronization strategy
- [ ] Advanced avatar animation
- [ ] Offline-first improvements
- [ ] Plugin/module architecture
- [ ] Stronger privacy model
- [ ] Ethical AI governance documentation
- [ ] Advanced context adaptation
- [ ] Public demo mode
- [ ] Contributor showcase
