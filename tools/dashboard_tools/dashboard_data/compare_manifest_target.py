...
DESCRIPTION :
Lit dashboard_data_manifest.json (etat courant) et dashboard_manifest_target.json
(cible projet), compare les fichiers attendus versus la cible, produit
dashboard_target_report.json et dashboard_target_report.md.
"""
...
CURRENT = DASHBOARD_DIR / "dashboard_data_manifest.json"
...
def main():
    if not TARGET.exists():
        print(f"[ALFRED] Fichier cible introuvable : {TARGET.name}")
        print(f"[ALFRED] Creez dashboard_manifest_target.json dans dashboard/dashboard_data/ pour activer la comparaison.")
        return
    if not CURRENT.exists():
        print(f"[ALFRED] Manifest courant introuvable : {CURRENT.name}")
        return
