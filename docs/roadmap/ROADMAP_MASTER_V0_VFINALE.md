...
- **Sprints réalisés** : pipeline conversationnel V1 (STT Whisper + LLM + TTS Piper) ; routeur LLM local-first Ollama→OpenAI→Anthropic ; function-calling réel branché au chat (23/07, corrige une hallucination d'UI) ; moteur de tâches complet + garde-fou anti-hallucination outils (24/07, ~35-40 % des appels d'outils llama3.2 n'aboutissaient à rien de réel avant le correctif) ; fixes TTS (élision, heure, double lecture) ; **chantier "sobriété/indépendance LLM externe" démarré le 14/08** — étude du document de cadrage `archi neuronale alfred.docx`, vision + plan en 4 points formalisés (`docs/architecture/vision_architecture_cognitive_alfred.md`), **P0 livré** : `SafetyNet` (`src/security/safety_gate.py`) bloque le repli vers OpenAI/Anthropic sur contenu sensible (santé, sécurité domicile) même si Ollama est indisponible — trou de confidentialité réel comblé avant le démarrage du Chantier 2 (données de santé Céline/Sébastien), commit `55ac5eee`. P1 (IntentNet déjà écrit mais jamais branché), P2 (sobriété modèle) et P3 (politique mémoire) restent à faire ; P4 (Cognitive Bus complet, Learning LLM) volontairement différé.
- **User stories clés** :
...
- **Chantier Knowledge/Training Dataset/Fine-tuning démarré le 21/08/2026** — axe distinct de la croissance du corpus RAG ci-dessus (388→3000 fichiers) : jusqu'ici le Knowledge était un système fermé, sans schéma de provenance ni Quality Gate, et aucun Training Dataset n'existait pour justifier un futur composant appris (motif du report P4 du 14/08 sur le chantier b01 sobriété LLM, ligne ci-dessus). Vision et plan en P0→P3 formalisés (`docs/architecture/vision_knowledge_training_finetuning_alfred.md`) : **P0 livré** — journalisation des échecs de repli cloud (Gap Dataset), Quality Gate, schéma de provenance additif sur les fiches existantes (commit `e95a3f87`) ; **P1 en cours** — constitution du Training Dataset (structure). P2 (Golden Dataset/évaluation) et P3 (premier fine-tuning LoRA/QLoRA, conditionné à un matériel qui n'existe pas encore — GPU NVIDIA dédié, cf. roadmap hardware Phase 2) restent à faire. Ne modifie pas les pourcentages de corpus RAG ci-dessus — c'est une couche de gouvernance/pipeline, pas des fiches supplémentaires.
...
| 10 | Chantier Knowledge/Training Dataset/Fine-tuning (b18, ajouté 21/08/2026) : P2-P3 restent à faire | 🟡 En cours | P0 (provenance/Quality Gate/Gap Dataset) livré 21/08 (commit `e95a3f87`), P1 (Training Dataset) en cours — voir `docs/architecture/vision_knowledge_training_finetuning_alfred.md`. P3 (fine-tuning réel) bloqué par du matériel qui n'existe pas encore (GPU NVIDIA dédié, cf. roadmap hardware Phase 2 ajustée le 21/08) — non prioritaire avant P1/P2. |
