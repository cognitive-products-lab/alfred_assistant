...
**Corrigé sur demande explicite (commits `4adb1f4`, `703b525`) :**
- `cadrage.html` : les 3 valeurs de gris incohérentes (`#8b949e` ×2, `#8892a0` ×1) unifiées sur `var(--muted)` (`#6B7280`), déjà établi et utilisé 9 fois dans ce même fichier pour le même rôle. Vérifié via serveur Flask réel : `getComputedStyle` confirme `rgb(107,114,128)` sur les 3, 0 erreur console.
- `apprentissages.html` : seule occurrence de `#8b949e` (palette GitHub-dark, étrangère à ce fichier) remplacée par `var(--text-dim)` (palette `style.css` du site, déjà utilisée ailleurs pour ce même rôle). Vérifié de la même façon, 0 erreur console.
...
- **Famille cyan/violet** (`--bg:#07090f`, `--acc:#00d4ff`, `--purple:#a78bfa`) : signalée ici comme non traitée dans un premier temps, puis corrigée sur demande explicite de Céline — voir section suivante.
...
### Famille cyan/violet ALFRED_PC (commit local `2996f7af`, non poussé)
Sur demande explicite de Céline de vérifier et corriger. Diff exact (pas de suppositions) confirmant une duplication réelle : `dashboard/ALFRED_DASHBOARD_DYNAMIC.html` et `dashboard/dashboard_data/ALFRED_DASHBOARD_DYNAMIC.html` ont un `:root` identique à l'octet ; `dashboard_gouvernance/dashboard_gouvernance.html`, `index.html` et `norm.html` ont un `:root` identique aux espaces/retours à la ligne près. Regroupé dans un nouveau `dashboard/dashboard-tokens-legacy.css` (13 variables), même méthode que la famille GitHub-dark. `--gui` (2 fichiers) et `--plan`/`--purple` (3 fichiers) restent locaux : même valeur `#a78bfa` mais noms différents entre les deux groupes, pas unifiés pour ne pas risquer de rater une référence `var()` existante ailleurs dans ces fichiers.

**Investigation du statut des fichiers (sans toucher au contenu, uniquement les tokens CSS) :**
- `dashboard/ALFRED_DASHBOARD_DYNAMIC.html` (racine) : aucun lanceur ne le référence (`paths.py` et `dashboard_data_server.bat` pointent tous les deux vers `dashboard_data/ALFRED_DASHBOARD_DYNAMIC.html`), et son fetch JSON cible un fichier qui n'existe pas et n'a jamais existé à cet endroit (vérifié) — très probablement orphelin. Non supprimé : c'est une décision de contenu/nettoyage, pas de style, hors périmètre de ce fix.
- `dashboard_gouvernance/dashboard_gouvernance.html` porte lui-même `VERSION: V1.0` dans son en-tête, contre `VERSION: V2.0 — architecture index + norm.html` pour `index.html` (le fichier réellement lancé par `dashboard_gouvernance_server.bat`, qui renvoie vers `norm.html`) : V1.0 semble remplacé par V2.0, mais non supprimé ici pour la même raison.

Vérifié via serveur HTTP local réel sur les 5 pages : `getComputedStyle` confirme des valeurs identiques à l'original, 0 nouvelle erreur (un 404 JSON préexistant sur le fichier racine orphelin, vérifié avant/après, non lié à ce changement).

**Reste à décider par Céline (hors périmètre technique) :** faut-il supprimer/archiver `dashboard/ALFRED_DASHBOARD_DYNAMIC.html` et `dashboard_gouvernance/dashboard_gouvernance.html` (V1.0), probablement obsolètes ?

### Bilan
26 fichiers modifiés au total sur 2 dépôts (16 sur `ALFRED_WEB` : 2 infra + 2 petits + 4 gros + 11 consolidation badge avec chevauchement + 2 dashboards résiduels + 2 gris cadrage/apprentissages ; 13 sur `ALFRED_PC` : 2 nouveaux fichiers de tokens + 6 dashboards GitHub-dark + 5 dashboards cyan/violet), 14 commits au total (11 sur `ALFRED_WEB`, tous poussés sur `origin/main` ; 3 sur `ALFRED_PC`, restés locaux). Aucune régression détectée (0 erreur console sur l'ensemble des pages vérifiées, valeurs calculées identiques à l'original à chaque étape). Les deux palettes (GitHub-dark et cyan/violet) sont désormais protégées par des tokens partagés partout où elles sont dupliquées de façon répétée et vérifiée. Reste ouvert, hors périmètre technique : décision de suppression/archivage sur 2 fichiers `ALFRED_PC` probablement obsolètes (signalés ci-dessus, pas de contenu touché). Chantier clos.
