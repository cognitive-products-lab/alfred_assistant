...
### Bilan
15 fichiers modifiés au total (2 infra + 2 petits + 4 gros + 11 consolidation badge, avec chevauchement + 2 dashboards résiduels), 9 commits, tous poussés sur `origin/main` d'`ALFRED_WEB`. Aucune régression détectée (0 erreur console sur l'ensemble des pages vérifiées, valeurs calculées identiques à l'original à chaque étape). La palette GitHub-dark est désormais protégée par des tokens partagés partout où elle est utilisée de façon répétée, à l'exception de `cadrage.html` (désordre local pré-existant, signalé) et `apprentissages.html` (cas isolé). Chantier clos.
